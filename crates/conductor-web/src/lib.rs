...
pub fn respond(repo: &Path, method: &Method, url: &str, body: &str) -> Reply {
    ...
    // The only writes: a person's decision on a stage the run is waiting for, and their
    // answer to a tool the agent asked for.
    if *method == Method::Post {
        return match parts.as_slice() {
            ["api", "runs", id, "decide"] if run_id_ok(id) => decide(repo, id, body),
            ["api", "runs", id, "answer"] if run_id_ok(id) => answer(repo, id, body),
...
fn answer(repo: &Path, id: &str, body: &str) -> Reply {
    let b: AnswerBody = match serde_json::from_str(body) {
        Ok(b) => b,
        Err(e) => return text(400, "text/plain; charset=utf-8", e.to_string()),
    };
    let dir = conductor_engine::store::RunDir::for_run(repo, id);
    ...
    match conductor_engine::ask::write_answer(&conductor_engine::ask::dir(&dir), b.ask, &a) {
        Ok(()) => json(Some(&a)),
        Err(e) if e.kind() == io::ErrorKind::AlreadyExists => {
            text(409, "text/plain; charset=utf-8", e.to_string())
        }
        Err(e) if e.kind() == io::ErrorKind::NotFound => {
            text(404, "text/plain; charset=utf-8", e.to_string())
        }
        Err(e) => text(500, "text/plain; charset=utf-8", e.to_string()),
    }
}
